...
class Is24ListingSource(
    ...
    override val platform = FlatPlatform.IS24
    ...
    override fun search(
        ...
        val result = try {
            ...
            val geocode = Is24Cities.geocode(filter.location?.city)
            ...
            NetworkResponseWrapper.success(Is24FlatMapper.mapSearch(json, filter.adType))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Is24BlockedException) {
            println("Is24ListingSource: ${e.message}")
    ...
    override fun detail(adId: Long): Flow<AppFlat?> = flow {
        val base = flatsDao.getById(platform, adId) ?: Is24FlatMapper.listStub(adId)
        emit(base)
        if (base.flatDevInfo.isDetailLoaded) return@flow
        try {
            val json = api.detail(adId.toString())
            val merged = Is24FlatMapper.mergeDetail(base, json)
            flatsDao.upsert(merged)
            emit(merged)
        } catch (e: CancellationException) {
            throw e
        } catch (e: Is24BlockedException) {
            println("Is24ListingSource.detail soft-fail $adId: ${e.message}")
        } catch (e: Exception) {
            println("Is24ListingSource.detail soft-fail $adId: ${e.message}")
            throw e
        }
