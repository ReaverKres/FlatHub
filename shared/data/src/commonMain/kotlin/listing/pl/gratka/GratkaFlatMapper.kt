...
object GratkaFlatMapper {
    ...
    private fun mapNode(node: JsonObject, adType: AdType): AppFlat {
        val idOnFrontend = node["idOnFrontend"].contentOrNull()
        ...
        val pricePln = node["price"].asObjectOrNull()?.get("amount").doubleOrNull()
        ...
        val phones = (person?.get("phones") ?: company?.get("phones"))
            .asArrayOrNull()
            ?.mapNotNull { it.contentOrNull() }
        val isOwner = person?.get("type").contentOrNull().equals("OWNER", ignoreCase = true)
