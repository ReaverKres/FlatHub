...
/**
 * Immowelt.at SSR HTML list (detail often DataDome-blocked). Soft-fail on captcha.
 */
class ImmoweltAtApiClient(
    private val httpClient: HttpClient,
) {
    suspend fun fetchSearchHtml(
        city: ImmoweltAtCities.CityPath,
        isSale: Boolean,
        page: Int,
    ): String {
        ...
        val url =
            "https://www.immowelt.at/suche/$op/wohnung/${city.path}/${city.geoId}$pageQs"
        val text = httpClient.get(url) {
            header(HttpHeaders.UserAgent, USER_AGENT)
            header(HttpHeaders.Accept, "text/html,application/xhtml+xml")
            header(HttpHeaders.AcceptLanguage, "de-AT,de;q=0.9,en;q=0.8")
            header(HttpHeaders.Referrer, "https://www.immowelt.at/")
        }.bodyAsText()
        ensureHtml(text)
        return text
    }
    ...
    private fun ensureHtml(text: String) {
        if (text.contains("geo.captcha-delivery.com", ignoreCase = true) ||
            ...
            throw ImmoweltAtBlockedException("Immowelt.at DataDome/captcha")
        }
    }
...
class ImmoweltAtBlockedException(message: String) : Exception(message)
